...
const ImageUploader: React.FC<ImageUploaderProps> = ({
  ...
  return (
    <div>
      ...
      <div
        {...getRootProps()}
        className={`border-2 border-dashed rounded-lg p-5 text-center cursor-pointer flex flex-col items-center justify-center
        ${isDragActive ? "border-blue-500 bg-blue-50" : "border-gray-300"}
        ${validationError ? "border-red-500" : ""}`}
      >
        <input {...getInputProps()} />
        <Image className="w-10 h-11 text-gray-400 " />
        {isDragActive ? (
